...
export default class Gizmo {
    constructor(scene, camera, renderer, orbit) {
        ...
        this.startPos = new THREE.Vector3();   // WORLD-space anchor of the drag
        this.startHit = new THREE.Vector3();
        this.startQuat = new THREE.Quaternion();
        this._wp = new THREE.Vector3();        // scratch for world<->local writes
    ...
    updateMatrix() {
        ...
        // Position the gizmo at the object's WORLD position, not its local
        // position - otherwise an object nested under a transformed parent
        // (e.g. a grouped entity offset to the selection's centroid) would
        // draw its gizmo at the local offset near the world origin. For a
        // top-level object (parent at identity) world == local, so this is a
        // no-op there and existing behaviour is unchanged.
        this.selectedObject.getWorldPosition(this.gizmoGroup.position);
        if (this.config.space === 'local') this.selectedObject.getWorldQuaternion(this.gizmoGroup.quaternion);
        else this.gizmoGroup.quaternion.set(0,0,0,1);
    }

    // Writes a WORLD-space position onto the selected object, converting into
    // its parent's local frame when it has a transformed parent. Mirror of
    // the getWorldPosition reads used to anchor the drag, so nested objects
    // move under the cursor instead of relative to the world origin.
    _setWorldPos(worldVec) {
        const p = this.selectedObject.parent;
        if (p && p !== this.scene) {
            p.updateWorldMatrix(true, false);
            this.selectedObject.position.copy(p.worldToLocal(this._wp.copy(worldVec)));
        } else {
            this.selectedObject.position.copy(worldVec);
        }
    }

    onPointerDown(e) {
        // Inert unless the host app is actively in editor mode. The game sets
        // window.editorModeActive true/false; the standalone Editor.html never
        // sets it (stays undefined), so the `=== false` test leaves that tool
        // untouched while stopping the gizmo from reacting during gameplay.
        if (typeof window !== 'undefined' && window.editorModeActive === false) return;
        this._updateMouse(e);
        ...
        if (this.selectedObject) {
            ...
            if (hits.length > 0) {
                ...
                this.selectedObject.getWorldPosition(this.startPos);
                ...
                this.dragPlane.setFromNormalAndCoplanarPoint(n, this.startPos);
                this.raycaster.ray.intersectPlane(this.dragPlane, this.startHit);
                this.dragOffset.copy(this.startPos).sub(this.startHit);
    ...
    onPointerMove(e) {
        ...
        if (this.raycaster.ray.intersectPlane(this.dragPlane, cur)) {
            if (this.currentMode === 'translate') {
                if (this.activeAxis === 'SCREEN') this._setWorldPos(cur.clone().add(this.dragOffset));
                else {
                    ...
                    if (this.activeAxis.length === 1) {
                        ...
                        const proj = d.dot(v);
                        this._setWorldPos(v.multiplyScalar(proj).add(this.startPos));
                    } else {
                        this._setWorldPos(d.add(this.startPos));
                    }
                }
    ...
    onPointerUp() {
        // Guard the orbit re-enable: this fires on EVERY canvas pointerup
        // (window listener), so without the gate a tap during normal gameplay
        // would flip the editor's OrbitControls back on - which, in player-
        // camera mode where controls drive the game camera, fought the game's
        // own follow-cam (the "camera keeps clicking while rotating" bug).
        if (typeof window !== 'undefined' && window.editorModeActive === false) return;
        this.activeAxis = null;
