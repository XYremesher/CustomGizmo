...
// Extracted verbatim from Editor/Editor.html's own ShapeGenerator (the
// standalone "Geometry Editor Pro" tool) so the in-game level editor
// (level_editor.js) can build/rebuild the same parametric primitives -
// see CLAUDE.md for why this was duplicated rather than imported from
// Editor.html directly (that file isn't a module other code can import
// from; its classes are trapped in inline <script type="module"> blocks).
export class ShapeGenerator {
    ...
    // Default param sets per shape type - extracted from Editor.html's own
    // EditorCore.getDefUserData (its cut/cap/flip fields default to
    // off/on exactly as they do there: cuts off, caps on, no offset).
    // `type` here is the ShapeGenerator key ('box'/'cyl'/'cone'/'sphere'/
    // 'torus'), not Editor.html's separate display-name ('Cube'/'Cylinder'/...).
    static defaultParams(type) {
        const base = {
            width: 2, height: 2, depth: 2,
            radius: type === 'box' ? 0 : 0.5, torusTube: 0.3, radialSegments: 32, cornerSegments: 8, heightSegments: 1,
            edgeThreshold: 1, cutX: false, capX: true, flipX: false, flatX: false, capFlatX: true, offsetX: 0,
        ...
        if (type === 'box') Object.assign(base, { radius: 0, cornerSegments: 3 });
        if (type === 'cyl') Object.assign(base, { radius: 0.5, radialSegments: 32, cornerSegments: 3 });
        if (type === 'cone') Object.assign(base, { radius: 0.5, radialSegments: 32, cornerSegments: 3 });
        if (type === 'sphere') Object.assign(base, { radialSegments: 32, cornerSegments: 16 });
        if (type === 'torus') Object.assign(base, { torusTube: 0.3, radialSegments: 16, cornerSegments: 32 });
        return base;
    }
}
